<template>
   ...
   <el-col :span="24" class="warp-breadcrum" style="height:50px;width:92%;margin:0px; position: absolute;top:0px;left:8%;line-height:50px;color:red">
    <el-breadcrumb separator="/" style="line-height:50px">
      ...
      <el-breadcrumb-item :to="{ path: '' }">体检完成</el-breadcrumb-item>
    </el-breadcrumb>
  </el-col>
 ...
 <div class="content_right">
  <div class="content_border">
   ...
   <span style="display:block;width:8%;height:20px;float:right;margin-top: 40px;">
    <!-- 上传凭证 -->
    <el-button type="text" v-show="ids == 3 || ids == 4 ? false:true" @click="nont(item.img_url,item.id)">
      <img style="height:24px;width:24px" src="../../assets/images/report.png" v-if="item.img_url !=null">
      <img style="height:20px;width:20px" src="../../assets/images/unreport.png" v-if="item.img_url ==null">
    </el-button>
...
medical:function(){
  ...
  axios.post(url+"/whjtys/physical/updataTJStatue?id="+this.id+"&url="+this.fileList2[0].url+"&loginId="+_this.loginId+"&token="+_this.token).then(function(response){
      _this.requestData()
      _this.fileList2 = []
      _this.$message({
  ...
  requestData:function(){
    let _this=this;
    let url = familyDoctor();
    axios.get(url+"/whjtys/physical/tjManagement?imgFlag="+this.value1+"&loginId="+_this.loginId+"&token="+_this.token).then(function(response){
  ...
  handleCurrentChange:function(val){
    ...
    let _this=this
    if(!_this.search){
      axios.get(url+"/whjtys/physical/tjManagement?pageNum="+val+"&imgFlag="+this.value1+"&loginId="+_this.loginId+"&token="+_this.token).then(function(response){
      _this.data3=response.data.pagehelper.list
    })
    }else{
      axios.get(url + "/whjtys/physical/tjManagement?searchKey=" + _this.search+"&pageNum="+val+"&imgFlag="+this.value1+"&loginId="+_this.loginId+"&token="+_this.token)
    ...
    Search:function(){
      let url = familyDoctor()
      let _this = this
      axios.get(url + "/whjtys/physical/tjManagement?searchKey=" + _this.search+"&imgFlag="+this.value1+"&loginId="+_this.loginId+"&token="+_this.token)
      .then(function(response) {
        let data = response.data.pagehelper
        _this.total=data.total
        _this.data3 = data.list
