  ...
  <div id="app">
   <el-col :span="24" class="warp-breadcrum" style="height:50px;width:92%;margin:0px; position: absolute;top:0px;left:8%;line-height:50px;color:red">
    <el-breadcrumb separator="/" style="line-height:50px">
      ...
      <el-breadcrumb-item :to="{ path: '' }">接种完成</el-breadcrumb-item>
    </el-breadcrumb>
  </el-col>
...
export default {
  data() {
    return {
      ...
      data3: [],
      value1: "",
      exportApi:{api:'/whjtys/vaccine/exportExcel/1',title:'完成'},
      loginId:"",
      token:"",
      options: [
        {value: 37,label: "儿童疫苗"},
        {value: 38,label: "宫颈癌疫苗"},
        {value: 39,label: "全部"}
      ],
    };
  },
  ...
  watch: {
    value1: function() {
      ...
      axios.get(url + "/whjtys/vaccine/JMOrderManager?falg=" + this.value1+"&searchKey=" + this.search)
        .then(response => {
  ...
  methods: {
    ...
    request: function() {
      let _this = this;
      let url = familyDoctor();
      axios.get(url + "/whjtys/vaccine/JMOrderManager?falg=" +_this.value1+"&loginId="+_this.loginId+"&token="+_this.token)
    ...
    handleCurrentChange: function(val) {
      ...
      if (!_this.search) {
        axios.get(
            url + "/whjtys/vaccine/JMOrderManager?pageNum=" + val + "&falg=" + this.value1+"&loginId="+_this.loginId+"&token="+_this.token)
          .then(function(response) {
        ...
        axios.get(
            url +
              "/whjtys/vaccine/JMOrderManager?searchKey=" +_this.search +"&pageNum=" + val +"&falg=" +this.value1+"&loginId="+_this.loginId+"&token="+_this.token
          )
          .then(function(response) {
            _this.data3 = response.data.pagehelper.list;
            _this.total = response.data.pagehelper.total;
    ...
    Search: function() {
      ...
      let _this = this;
      axios
        .get(url + "/whjtys/vaccine/JMOrderManager?searchKey=" + _this.search+"&falg=" +this.value1+"&loginId="+_this.loginId+"&token="+_this.token)
        .then(function(response) {
          _this.data3 = response.data.pagehelper.list;
          _this.total = response.data.pagehelper.total;
        });
...
.content > div > div.content_left > dl > dd {
  height: 21px;
  line-height: 23px;
}
.content_right {
